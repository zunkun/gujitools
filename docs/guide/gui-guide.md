...
列表最左边的**「序号」就是任务编号**（`0001`、`0002`…），与
`~/Documents/guji/tasks/<编号>` 目录一一对应；排序、搜索、翻页都不会改变它，
所以随时可以按号去找磁盘上的任务目录。
...
| 目标 DPI dpi   | **整页渲染**的分辨率下限（默认 300）    | 矢量/普通 PDF 保持 `300`；想回到旧的 72 DPI 才填 `72`         |
...
- 中间是**版面编辑器**：按右侧当前参数算出这一页图片在纸上的位置与大小，
  **直接拖动图片移动、拖四角缩放**，坐标会记进任务（`print.json`），
...
> **标题恒在纸张上边、页码恒在下边，且都是竖排**——这两项在界面上**不提供选择**
> （古籍定式：书名在版框之上、页码在版心之下）。想改成别的位置/方向，只能改
> 配置文件里的 `title_position` / `title_orientation`（见 `docs/functions/print.md`）。
...
| 行                  | 含义                                                                  |
| ------------------- | --------------------------------------------------------------------- |
| **左右边距**        | 一个值同时管两边：左页贴左纸边、右页贴右纸边，**左右是对称的**（口径是**文字轮廓边缘**到纸边，所以右页的落点会自动往左退一个字宽来兑现它） |
...
- 标题和页码**都固定竖排**：界面不再有「位置」「文字方向」两个下拉——既然
  标题只可能在上、页码只可能在下，摆着这两个选项只会跟上面两行自相矛盾
  （用户要么得做动态联动，要么干脆不要这两个参数，这里选了后者）；
- 两个输入框都是**通栏宽**（和「古籍名称」「PDF 文件名」一样宽），
  看得清、也好点；
- 不填时不启用「自定义」= 老行为：横向贴页边距中缝、纵向内缩 2mm
  （**不是** 2mm 这个数，而是由页边距推导，见 `docs/functions/print.md`）；
...
点 **「生成PDF」** 生成 PDF，输出在任务目录的 `stages/pdf/` 下。
