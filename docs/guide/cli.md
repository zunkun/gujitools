...
> ⚠️ 标注图是**全分辨率无损位图**：一本 320 页的书能写到 2–9GB（随原图大小），
> 只想核对检测框时加 `--ext jpg` 可把体积降到约 1/10。
...
| `--workers`   | 见下     | 并行线程数。默认上限 **4**（一页 5000×4400 的图约需 350MB，线程数直接乘成峰值内存）；**`detect` 例外，上限 8**（每张只在常驻服务里读一次图，远比去底/裁剪轻） |
