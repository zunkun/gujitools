...
- 默认 1440×920，最小 1080×720，标题「guji 古籍处理平台」；QStackedWidget 切换
...
![任务列表页](screenshots/01-任务列表页.png)
...
![详情页 · 提取](screenshots/02-详情页-提取.png)

- 顶部信息卡、预览区、控制列各自是独立 `Card`；步骤条与底部日志状态条直接画在
  页面底色上（状态条只有一条 1px 上分隔线，不套卡片）。
- 预览区占满剩余宽度；控制列限宽 340–440px（第四步的 YAML 表单放宽到 400–580px，
...
| 检测文本框 | 图片去底色 |
| --- | --- |
| ![详情页 · 检测](screenshots/03-详情页-检测.png) | ![详情页 · 去底色](screenshots/04-详情页-去底色.png) |

![详情页 · 生成 PDF](screenshots/05-详情页-生成PDF.png)

日志浮层（点击底部状态条唤出）：

![详情页 · 日志浮层](screenshots/06-详情页-日志浮层.png)

> 截图由 `tests/gui_shot.py` 离屏渲染生成（演示数据，文件名与 `docs/gui/screenshots/`
