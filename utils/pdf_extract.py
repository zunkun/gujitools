...
# ------------------------------------------------------------------ 分辨率
# 单页输出宽度的硬上限（px）。再大的页面会吃光内存，且下游（rembg/print）
# 也处理不动，所以一律按宽度封顶。
MAX_OUTPUT_WIDTH_PX = 6000

# 页面宽度超过这个 pt 值就认为"已经够宽"，不再按 zoom 放大。
WIDE_PAGE_PT = 3000

# 整页渲染的**目标 DPI 下限**。
#
# 背景（72 DPI 陷阱）：PDF 的页面单位是 pt（1pt = 1/72 inch），PyMuPDF 在
# zoom=1 时 1pt 渲染成 1px —— 也就是 72 DPI。扫描古籍无所谓：quick 模式会
# 直接取内嵌的原图字节，分辨率由扫描件本身决定。但**普通矢量 PDF**（没有
# 内嵌图，或内嵌图不被 quick 接受）只能整页渲染，zoom=1 就只有 72 DPI，
# 去底色后再嵌回打印 PDF 必然发虚。
#
...
def calculate_zoom(page_width: float, requested_zoom: float = 1) -> float:
    ...
    # 限制最大输出宽度，防止内存溢出
    if target_width > MAX_OUTPUT_WIDTH_PX:
        return max(1, MAX_OUTPUT_WIDTH_PX // int(page_width))
    return requested_zoom


def render_zoom(page_width: float, requested_zoom: float = 1,
                dpi: float = DEFAULT_RENDER_DPI) -> float:
    ...
    返回:
        实际渲染缩放因子。
    """
    base = calculate_zoom(page_width, requested_zoom)
    if not dpi or page_width <= 0:
        return base
    # 下限也要受封顶约束：页面本身就极宽时，6000px 上限反而更小
    cap = max(1.0, MAX_OUTPUT_WIDTH_PX / float(page_width))
    return max(base, min(dpi / 72.0, cap))
...
def extract_pdf_optimized(
    ...
    try:
        ...
        try:
            ...
            doc.close()
        actual_zoom = render_zoom(page_width, zoom, dpi)
        ...
        print(f"🔧 实际 zoom: {actual_zoom}（内嵌图路径按原图，不受 DPI 下限影响）")
