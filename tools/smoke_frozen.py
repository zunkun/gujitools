...
def main() -> int:
    ...
    check("guji-gui.exe 存在", (dist / "guji-gui.exe").is_file())
    if not cli.is_file():
    ...
    gui = dist / "guji-gui.exe"
    if not gui.is_file():
        check("guji-gui.exe 存在", False)
    else:
        ...
        env = dict(os.environ, QT_QPA_PLATFORM="offscreen",
                   GUJI_GUI_SELFTEST="1")
        proc = subprocess.Popen([str(gui)], cwd=str(PROJECT_ROOT), env=env,
    ...
    with tempfile.TemporaryDirectory(prefix="guji_smoke_") as tmp:
        ...
        check("crop 正常退出（torch + YOLO 可用）", r.returncode == 0,
              (r.stderr or "")[-400:])
