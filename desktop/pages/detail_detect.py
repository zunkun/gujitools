...
框坐标持久化在 detect_boxes 表（键为页面 stem）：
- 选中图片时优先读库（DB 命中不再检测，手动框不被自动结果覆盖）；
- 子进程检测到的框入库（origin=auto）；
- 预览区拖动线框后入库（origin=manual），全程不生成新文件。
"""
...
class DetectMixin:
    ...
    def _start_detect(self, path: Path) -> None:
        ...
        if getattr(sys, "frozen", False):
            ...
            self.detect_process.setWorkingDirectory(str(Path(__file__).parents[2]))
